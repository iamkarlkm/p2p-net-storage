...
/**
 * COS 分片上传处理器：负责读取单个分片并上传到 COS。
 *
 * <p>作为 Runnable 在多线程上传时被调度执行。</p>
 */
public class CosFileSegmentsPutProcessor implements Runnable {
	...
	public CosFileSegmentsPutProcessor(FileSegmentsDataModel model, RandomAccessFile infoFile, CountDownLatch countDownLatch, CountDownLatch errorCountDown, AtomicBoolean errorTag) {
		this.model = model;
	...
	public CosFileSegmentsPutProcessor( FileSegmentsDataModel model,File data, RandomAccessFile infoFile, CountDownLatch countDownLatch, CountDownLatch errorCountDown, AtomicBoolean errorTag) {
		this.model = model;
		this.countDownLatch = countDownLatch;
		this.errorCountDown = errorCountDown;
		this.errorTag = errorTag;
    ...
    public void run() {
		try {
			...
			P2PCosUtils.putFileSegment(model);
			FileUtil.concurentAppend(infoFile, (model.blockIndex+"\n").getBytes());
		} catch (Exception ex) {
			Logger.getLogger(CosFileSegmentsPutProcessor.class.getName()).log(Level.SEVERE, null, ex);
			errorTag.set(true);
		}finally{
