...
public abstract class AbstractUdpMessageProcessorByteBuf extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    public AbstractUdpMessageProcessorByteBuf() {
        //this.magic = magic;
        //this.queueSize = queueSize;
    }

    protected abstract void processMessage(ChannelHandlerContext ctx, DatagramPacket datagramPacket, P2PWrapper message);
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        if (frameLengthInt == -1) { // new frame
            ...
            magic = in.readInt();
            if (in.capacity() < frameLengthInt) {//调整缓冲区
        ...
        if (magic == attrMagic.get()) {
            System.out.println("data rix:"+in.readerIndex());
            System.out.println("readableBytes:"+in.readableBytes());
            ...
            try {//先获取可读字节数
                P2PWrapper request = SerializationUtil.deserialize(P2PWrapper.class, in, magic, frameLengthInt);
...
//                System.out.println("Server echo -> test udp-0".length());
                ...
                log.warn("{}消息解析异常:{},关闭channel:{}", in, ex, ctx.channel().id());
//                try {
//                    ctx.close();
//                } catch (Exception ex2) {
//                    log.error(ex.getMessage());
//                }
                //throw new RuntimeException("消息解析异常->", ex);
            } finally {
    ...
    public void sendResponse(ChannelHandlerContext ctx, DatagramPacket datagramPacket, P2PWrapper response) {
        ...
        if (magicChannel != null) {
            sendResponse(ctx, datagramPacket, response, magicChannel);
        } else {
            sendResponse(ctx, datagramPacket, response, magic);
        }
    }
     ...
     * @param magic
     */
    public static void sendResponse(ChannelHandlerContext ctx, DatagramPacket datagramPacket, P2PWrapper response, int magic) {
        try {

            ByteBuf buffer = SerializationUtil.serializeToByteBuf(response, magic);
            //收到udp消息后，可通过此方式原路返回的方式返回消息，例如返回时间戳
            ChannelFuture cf = ctx.writeAndFlush(new DatagramPacket(buffer, datagramPacket.sender()));
...
//            InetSocketAddress t = datagramPacket.sender();
     ...
     * @param response
     */
    public static void sendResponse(Channel channel, P2PWrapper response) {
        try {
            Attribute<Integer> attrMagic = channel.attr(ChannelUtils.MAGIC);
            ByteBuf buffer = SerializationUtil.serializeToByteBuf(response, attrMagic.get());
            //收到udp消息后，可通过此方式原路返回的方式返回消息，例如返回时间戳
                ...
                cf.addListener(new ChannelFutureListener() {
                    public void operationComplete(ChannelFuture future) {
                        ...
                        if (future.isSuccess()) {
                            ...
                            log.warn("{}消息处理未成功,可能原因:{},关闭channel:{}", response, future.cause().getMessage(), channel.id());
                            channel.close();
                        }
                    }
                });
    ...
    public void sendPongMsg(ChannelHandlerContext ctx, DatagramPacket datagramPacket, int magic) {
        ...
        sendResponse(ctx, datagramPacket, HEART_PONG, magic);
