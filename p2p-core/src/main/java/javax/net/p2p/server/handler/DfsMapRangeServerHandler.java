...
public class DfsMapRangeServerHandler implements P2PCommandHandler {
    @Override
    public P2PCommand getCommand() {
        return P2PCommand.DFS_MAP_RANGE;
    }
    ...
    public P2PWrapper process(P2PWrapper request) {
        ...
        if (!(request.getData() instanceof DfsMapRangeReq req)) {
            return P2PErrors.stdError(request.getSeq(), P2PErrorCode.INVALID_REQUEST, "invalid DFS_MAP_RANGE payload");
        }
        return P2PWrapper.build(request.getSeq(), request.getCommand(), backend.handleRange(req));
    }
}
