...
public class DfsMapGetTopologyServerHandler implements P2PCommandHandler {
    @Override
    public P2PCommand getCommand() {
        return P2PCommand.DFS_MAP_INT_GET_TOPOLOGY;
    }
    ...
    public P2PWrapper process(P2PWrapper request) {
        ...
        if (!(request.getData() instanceof DfsMapGetTopologyReq req)) {
            return P2PErrors.stdError(request.getSeq(), P2PErrorCode.INVALID_REQUEST, "invalid DFS_MAP_INT_GET_TOPOLOGY payload");
        }
        return P2PWrapper.build(request.getSeq(), request.getCommand(), backend.handleGetTopology(req));
    }
}
