...
public class AuthHandshakeModesUdpTest {
    ...
    private void testOneMode(String cryptoMode) throws Exception {
        int port = randomUdpPort();
        File yamlFile = null;
        File keyDir = null;
        P2PServerUdp server = null;
        P2PClientUdp client = null;
        try {
            ...
            keyDir = new File("target/auth-keys-udp-" + cryptoMode + "-" + port);
            keyDir.mkdirs();
            ...
            String yaml = ""
                ...
                + "keyDir: \"auth-keys-udp-" + cryptoMode + "-" + port + "\"\n"
                + "client:\n"
                + "  userId: \"" + userId + "\"\n"
            ...
            yamlFile = new File("target/auth-test-udp-" + cryptoMode + "-" + port + ".yaml");
            ...
            server = P2PServerUdp.getInstance(P2PServerUdp.class, port);
            Thread.sleep(400);
            client = new P2PClientUdp(new InetSocketAddress("127.0.0.1", port));
