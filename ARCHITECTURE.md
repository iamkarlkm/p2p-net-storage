...
本网络的核心思想之一是：这是一个严格管控的网络。系统按“默认拒绝（deny-by-default）”设计，所有关键输入都必须显式被识别与允许，否则拒绝处理或断开连接。

强管控覆盖范围（四个“未知”）：

- 未知用户：未通过握手/登录/授权的用户请求一律拒绝
- 未知节点：未知节点身份、未知节点公钥/NodeKey、未知拓扑来源一律拒绝或隔离
- 未知服务：未启用或未加载的服务类别一律拒绝（service unavailable）
- 未知数据格式/未知命令：无法解析、未注册、未在允许列表内的命令/数据一律拒绝（invalid/unknown）

### 管控落地（机制与边界）

- 身份与握手：连接必须先完成握手协商安全参数；缺少握手密钥材料/协商失败直接拒绝
- 认证与授权：命令层面按用户 ID 做 allowlist；未登录或无权限直接拒绝
- 命令注册：仅允许已注册的命令处理器；未知命令不进入业务逻辑
- 数据格式：业务数据必须为协议约定的结构化载荷（例如 protobuf/Protostuff 显式模型）；解析失败视为无效数据
- 服务类别：每个命令属于一个服务类别（P2PServiceCategory）；服务禁用时统一返回 service unavailable，并可按类别卸载/加载 handler
- 文件与存储：文件路径必须严格沙箱化（storeId 注册、拒绝路径穿透、拒绝符号链接逃逸）
- 版本与兼容：magic/version/flags 等协议参数用于拒绝不兼容或伪造流量
...
*本文档描述p2p-net-storage项目各模块之间的详细交互关系，包括数据流、控制流、异常处理、性能优化等方面，为项目开发和维护提供完整的架构参考。*
