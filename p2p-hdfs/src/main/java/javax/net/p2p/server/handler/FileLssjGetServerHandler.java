 ...
 */
public class FileLssjGetServerHandler implements P2PCommandHandler {

	@Override
	public P2PCommand getCommand() {
		return P2PCommand.GET_LSSJ_FILE;
	}
	...
	public P2PWrapper process(P2PWrapper request) {
		try {
			if (request.getCommand().getValue() == P2PCommand.GET_LSSJ_FILE.getValue()) {
				LssjImageModel payload = (LssjImageModel) request.getData();
				//new File("E:/VEH_IMAGES");
				File parent = SharedStorage.getStorageLocation(payload.storeId);
				//               if(parent == null){
				//                   return P2PWrapper.builder(request.getSeq(),P2PCommand.STD_ERROR, "存储ID对应目录不存在 -> "+payload.storeId);
				//               }
