...
public class DbRowCountServerHandler implements P2PCommandHandler {
    @Override
    public P2PCommand getCommand() {
        return P2PCommand.DB_ROW_COUNT;
    }

    @Override
    public P2PWrapper process(P2PWrapper request) {
        try {
            if (request.getCommand() != P2PCommand.DB_ROW_COUNT) {
                return P2PErrors.stdError(request.getSeq(), P2PErrorCode.ROUTING_HANDLER_MISMATCH);
            }
            if (!(request.getData() instanceof DbRowCountRequest payload)) {
            ...
            return P2PWrapper.build(request.getSeq(), P2PCommand.R_OK_DB_ROW_COUNT, new DbRowCountResponse(count));
