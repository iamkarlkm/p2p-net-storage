 ...
 * </ul>
 */
public class DsHashMapI64 extends DsObject implements Map<Long, Long> {
...
//    private static final int NODE_SIZE = BITMAP_BYTES + 256 * SLOT_PAYLOAD_BYTES;
    ...
    private int keyStoreSize = 2;
    
    
    private final DsObject keyStore;
    
    private final DsHashMapI64 nextHashMap;
     ...
     * @param file
     */
    public DsHashMapI64(File file) {
        super(file, HEADER_SIZE,2624);//2字节索引 8位哈希 => 256*2-bit(位图)/8 + 256*2-byte(key) + 256*8-byte(value) = 64+512+2048 =2624
        ...
        File key64File = new File(file.getAbsolutePath()+".k64");
        DsObject keyStore64 = new DsObject(key64File,8);//2^64个key(8字节64位)索引
        ...
        DsHashMapI64 nextHashMap64 = new DsHashMapI64(nextHashMap64File,4,4,4160,8,keyStore64,null);//64位哈希是目前实现最后一级
        ...
        nextHashMap = new DsHashMapI64(nextHashMap32File,2,2,3136,4,keyStore32,nextHashMap64);
    }
    


    public DsHashMapI64(File file,int hashOffset,int hashLen,int dataSize,int keyStoreSize,DsObject keyStore32,DsHashMapI64 nextHashMap) {
    ...
    public Set<Long> keySet() {
        return new AbstractSet<Long>() {
            ...
            @Override
            public int size() {
                return DsHashMapI64.this.size();
            }

            @Override
            public boolean contains(Object o) {
                return DsHashMapI64.this.containsKey(o);
            }

            @Override
            public boolean remove(Object o) {
                return DsHashMapI64.this.remove(o) != null;
            }

            @Override
            public void clear() {
                DsHashMapI64.this.clear();
            }
        };
    }
            ...
            public Iterator<Entry<Long, Long>> iterator() {
                ...
                return new Iterator<Entry<Long, Long>>() {
                    ...
                    public Entry<Long, Long> next() {
                        ...
                        return new Entry<Long, Long>() {
                            ...
                            public Long getValue() {
                                return DsHashMapI64.this.get(entryKey);
                            }

                            @Override
                            public Long setValue(Long value) {
                                return DsHashMapI64.this.put(entryKey, value);
                            }
                    ...
                    public void remove() {
                        ...
                        DsHashMapI64.this.remove(lastKey);
                        canRemove = false;
                    }
                };
            }
    ...
    private long loadKeyIndexOffset(long position) throws IOException {
        return switch (keyStoreSize) {
            case 2 -> loadU16ByOffset(position);
            case 4 -> loadU32ByOffset(position);
            case 8 -> loadLongOffset(position);
            default -> throw new RuntimeException("invalid keyStoreSize -> " + keyStoreSize);
        };
    }
    ...
    private long valuePos(long nodeId, int slot) {
        return nodeBase(nodeId) + BITMAP_BYTES + keyStoreSize*256 + (long) slot * SLOT_PAYLOAD_BYTES;
    }
    
    private long keyPos(long nodeId, int slot) {
        return nodeBase(nodeId) + BITMAP_BYTES + (long) slot * keyStoreSize;
    }
