...
//    private static final int NODE_SIZE = BITMAP_BYTES + 256 * SLOT_PAYLOAD_BYTES;
    ...
    public DsHashMap(File file) {
        ...
        File file64 = new File(file.getAbsolutePath()+".k64");
        DsObject keyStore64 = new DsObject(file64,8);//2^64个key(8字节64位)索引
        
        File file64m = new File(file.getAbsolutePath()+".m64");//8字节索引 8位哈希 => 256*2bit(位图)/8 + 256*8byte(key) + 256*8byte(value) = 64+512+2048 =4160
        DsHashMap dsHashMap64 = new DsHashMap(file64m,0,8,4160,8,keyStore64);
        
        File file32m = new File(file.getAbsolutePath()+".m32");//4字节索引 8位哈希 => 256*2bit(位图)/8 + 256*4byte(key) + 256*8byte(value) = 64+512+2048 =3136
        File file32 = new File(file.getAbsolutePath()+".k32");
        DsObject keyStore32 = new DsObject(file32,8);//2^31个key(8字节64位)索引
        nextHashMap = new DsHashMap(file32m,0,4,3136,4,keyStore32,dsHashMap64);
    }
     ...
     * @param keyStore
     */
    public DsHashMap(File file,int hashOffset,int dataSize,DsObject keyStore) {
        super(file, dataSize);
        zeroNode = new long[this.dataUnitSize / 8];
        initHeader();
     ...
     * @param keyStore
     */
    public DsHashMap(File file,int hashOffset,int hashLen,int dataSize,int keyStoreLen,DsObject keyStore) {
        super(file, dataSize);
        zeroNode = new long[this.dataUnitSize / 8];
        initHeader();
        this.hashOffset = hashOffset;
     ...
     * @param nextHashMap
     */
    public DsHashMap(File file,int hashOffset,int dataSize,int keyStoreLen,DsObject keyStore32,DsHashMap nextHashMap) {
        super(file, dataSize);
        zeroNode = new long[this.dataUnitSize / 8];
        initHeader();
        this.hashOffset = hashOffset;
    ...
    public DsHashMap(File file,int hashOffset,int hashLen,int dataSize,int keyStoreLen,DsObject keyStore32,DsHashMap nextHashMap) {
        super(file, dataSize);
        zeroNode = new long[this.dataUnitSize / 8];
        initHeader();
        this.hashOffset = hashOffset;
                ...
                if (level < hashLen - 1) {
                    switch (state) {
                        ...
                        case STATE_VALUE -> {
                            ...
                            long oldKey =  keyStore.readLong(oldKeyId);
     ...
     * @throws java.io.IOException
     */
    private Long putInner(byte[] hashes,long key, long value,int currentLevel) throws IOException {
       long nodeId = 0;
            for (int level = currentLevel; level < hashLen; level++) {
                ...
                if (level < hashLen - 1) {
                    switch (state) {
                        ...
                        case STATE_VALUE -> {
                            //slot有值,深入下一层
                            int keyIndex = loadU16ByOffset(keyPos(nodeId, slot));
                            long oldKey =  keyStore.readLong(keyIndex);
                            long oldValue = loadLongOffset(valuePos(nodeId, slot));
                            if(oldKey == key){
                                if(oldValue != value){// Key同，值不同，更新。
                                    storeLongOffset(valuePos(nodeId, slot), value);
                            ...
                            //深入下一层,分别存储两个值。
                            int nextLevel = level+1;
                            putInner(hashes,oldKey, oldValue, nextLevel);
                            putInner(hashes,key, value, nextLevel);
                            return null;
                        }
                        default -> {
                             throw new IOException("invalid state: " + state);
                ...
                switch (state) {
                    ...
                    case STATE_VALUE -> {
                        long keyIndex = 0;
                        switch(keyStoreLen){
                            case 2 -> keyIndex = loadU16ByOffset(keyPos(nodeId, slot));
                            case 4 -> keyIndex = loadU32ByOffset(keyPos(nodeId, slot));
                            case 8 -> keyIndex = loadLongOffset(keyPos(nodeId, slot));
            ...
            if (level < hashLen - 1) {
                while (true) {
                    ...
                    if (state == STATE_VALUE) {
                        ...
                        long storedKey = keyStore.readLong(keyId);
                        if (storedKey != key) {
                            return null;
                        }
                        return v;
    ...
    private void collectKeys(Longs out, long nodeId, int level) throws IOException {
        for (int slot = 0; slot < 256; slot++) {
            ...
            switch (state) {
                ...
                case STATE_VALUE -> {
                    long keyId = loadKeyIndexOffset(keyPos(nodeId, slot));
                    long key = keyStore.readLong(keyId);
                    out.add(key);
                }
                case STATE_CHILD -> {
                    long child = loadLongOffset(valuePos(nodeId, slot));
