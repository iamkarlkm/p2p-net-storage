    ...
    static class DbMasterIndex extends DsObject {
        ...
        public void put(byte[] sha256, long indexId) throws IOException {
            ...
            while (true) {
                ...
                if (buffer.getLong(bufferOffset) != 0) isEmpty = false;
                
                if (isEmpty) {
                     for (int i = 0; i < 32; i++) {
                        if (buffer.get(bufferOffset + i) != 0) {
                            isEmpty = false;
                            break;
                        }
                    }
                ...
                if (isEmpty) {
                    ...
                    // 写入 32 字节 Key
                    for (int i = 0; i < 32; i++) {
                        buffer.put(bufferOffset + i, sha256[i]);
                    }
                    // 写入 8 字节 Value
                    buffer.putLong(bufferOffset + 32, indexId);
                    dirty(bufferIndex);
                    return;
                } else {
                    // 检查是否是同一个 Key
                    boolean match = true;
                    for (int i = 0; i < 32; i++) {
                        if (buffer.get(bufferOffset + i) != sha256[i]) {
                            match = false;
                            break;
                        }
                    }
        ...
        public Long get(byte[] sha256) throws IOException {
            ...
            while (true) {
                ...
                if (match) {
                    return buffer.getLong(bufferOffset + 32);
                }
