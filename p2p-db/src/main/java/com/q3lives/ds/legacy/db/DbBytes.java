...
public class DbBytes {
    ...
    /**
     * 触发对所有 DbBytes 实例的 bucket 进行 free 文件紧缩（compaction）。
     *
     ...
     * </ul>
     */
    public DbBytes(String rootDir) throws IOException {
    ...
    public long update(byte[] key, byte[] value) throws IOException, InterruptedException {
        ...
        byte[] md5 = DsDataUtil.md5(key);
        return put(key, md5, value);
    }
    ...
    public byte[] getKeyByValue(byte[] value) throws IOException, InterruptedException {
        byte[] valMd5 = DsDataUtil.md5(value);
     ...
     * 根据值 MD5 反推 Key。
     */
    public byte[] getKeyByValueMd5(byte[] valMd5) throws IOException, InterruptedException {
        // 1. 查询值索引 -> IndexID
    ...
    public List<Long> getChunkList(byte[] key, int page, int pageSize) throws IOException, InterruptedException {
        byte[] md5 = DsDataUtil.md5(key);
        return getChunkList(key, md5, page, pageSize);
    }
