...
public class DbString {
    ...
    static {
        scheduler = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "DbString-Compaction-Thread");
            t.setDaemon(true);
            return t;
        });
    ...
    private static final java.util.List<DbString> instances = new java.util.ArrayList<>();

    /**
     * 触发对所有 DbString 实例的 bucket 进行 free 文件紧缩（compaction）。
     *
     * <p>该方法由定时任务每天调用一次，也可以由外部显式调用。</p>
     */
    public static void compactAll() {
        synchronized(instances) {
            for (DbString db : instances) {
                db.compactBuckets();
            }
        }
    }
    ...
    public static final int INDEX_FIXED_PART_SIZE = 32;
    ...
    public static final int INDEX_OFFSET_MD5 = 16;
    /** Key 内容在 Index Data 中的偏移量 */
    public static final int INDEX_OFFSET_KEY_CONTENT = 32;
    /** ID 编码中，Offset 占用的位数 */
     ...
     * <p>会初始化两个总控索引：</p>
     * <ul>
     *   <li>masterIndex：key 的 MD5 -> indexId</li>
     *   <li>valueIndex：value 的 MD5 -> indexId（用于按 value 反查）</li>
     * </ul>
     */
    public DbString(String rootDir) throws IOException {
        this.rootDir = rootDir;
        File root = new File(rootDir);
        if (!root.exists()) root.mkdirs();
        
        // 初始化总控 MD5 索引 (Key MD5 -> IndexID)
        File masterFile = new File(root, INDEPENDENT_SPACE + File.separator + "master_md5.idx");
        ...
        File valueIndexFile = new File(root, INDEPENDENT_SPACE + File.separator + "value_md5.idx");
     ...
     * @return 64位 IndexID
     */
    public long update(String key, String value) throws IOException, InterruptedException {
     ...
     * @return 64位 IndexID
     */
    public long put(String key, String value) throws IOException, InterruptedException {
        byte[] keyBytes = key.getBytes(StandardCharsets.UTF_8);
        byte[] valBytes = value.getBytes(StandardCharsets.UTF_8);
        // 计算 MD5
        byte[] md5 = DsDataUtil.md5(keyBytes);
        return put(keyBytes, md5, valBytes);
    }
     ...
     * </p>
     */
    public long put(byte[] keyContent, byte[] md5, byte[] value) throws IOException, InterruptedException {
        // 1. 存储 Value
        long valueId;
        ...
        System.arraycopy(md5, 0, indexData, INDEX_OFFSET_MD5, 16);
        ...
        // 4. 更新总控 MD5 索引
        masterIndex.put(md5, indexId);
        
        // 5. 更新值索引 (Value MD5 -> IndexID)
        byte[] valueMd5 = DsDataUtil.md5(value);
        valueIndex.put(valueMd5, indexId);
    ...
    public String get(String key) throws IOException, InterruptedException {
        byte[] keyBytes = key.getBytes(StandardCharsets.UTF_8);
        byte[] md5 = DsDataUtil.md5(keyBytes);
        byte[] valBytes = get(keyBytes, md5);
        return valBytes == null ? null : new String(valBytes, StandardCharsets.UTF_8);
    }
     ...
     */
    public String getKeyByValue(String value) throws IOException, InterruptedException {
        byte[] valBytes = value.getBytes(StandardCharsets.UTF_8);
        byte[] valMd5 = DsDataUtil.md5(valBytes);
        return getKeyByValueMd5(valMd5);
    }

    /**
     * 根据值 MD5 反推 Key。
     */
    public String getKeyByValueMd5(byte[] valMd5) throws IOException, InterruptedException {
        // 1. 查询值索引 -> IndexID
        Long indexId = valueIndex.get(valMd5);
        ...
        return new String(keyContent, StandardCharsets.UTF_8);
    }
     ...
     */
    public List<Long> getChunkList(String key, int page, int pageSize) throws IOException, InterruptedException {
        byte[] keyBytes = key.getBytes(StandardCharsets.UTF_8);
        byte[] md5 = DsDataUtil.md5(keyBytes);
        return getChunkList(keyBytes, md5, page, pageSize);
    }

    /**
     * 获取超大数据的分段 ID 列表（默认 page=1，pageSize=5000）。
     */
    public List<Long> getChunkList(String key) throws IOException, InterruptedException {
     ...
     * 获取超大数据的分段ID列表（核心逻辑）。
     */
    public List<Long> getChunkList(byte[] keyContent, byte[] md5, int page, int pageSize) throws IOException, InterruptedException {
        // 1. 查询总控 MD5 索引 -> IndexID
        Long indexId = masterIndex.get(md5);
        if (indexId == null) return null;
        ...
        // 校验 MD5
        byte[] storedMd5 = new byte[16];
        System.arraycopy(indexBucketData, INDEX_OFFSET_MD5, storedMd5, 0, 16);
        if (!Arrays.equals(md5, storedMd5)) return null;
     ...
     * 获取数据（核心逻辑）。
     */
    public byte[] get(byte[] keyContent, byte[] md5) throws IOException, InterruptedException {
        // 1. 查询总控 MD5 索引 -> IndexID
        Long indexId = masterIndex.get(md5);
        if (indexId == null) return null;
     ...
     */
    public String scanValueByKey(String key) throws IOException {
        ...
        for (long offset = HEADER_SIZE; offset < maxOffset; offset += bucketSize) {
            ...
            if (Arrays.equals(keyContent, storedKey)) {
                ...
                return new String(realValue, StandardCharsets.UTF_8);
            }
        }
     ...
     */
    public long scanValueIdByValue(String value) throws IOException {
        byte[] valueContent = value.getBytes(StandardCharsets.UTF_8);
     ...
     * @return 对应的 Key
     */
    public String scanKeyByValue(String value) throws IOException {
    ...
    /**
     * 通过 indexId 直接删除 index/value 对应的 bucket 单元（把 offset 加入 free 列表）。
     *
     ...
     *
     * <p>会先读 index record 得到 valueLen/valueId，再读 value bucket 并截取 storedValueLen。</p>
     */
    ...
    private byte[] getData(long id, String type) throws IOException {
     ...
     */
         ...
         * <p>会初始化：</p>
         * <ul>
         *   <li>data 文件头（magic/nextOffset）</li>
         *   <li>free 文件（.free）：存放可复用 offset 的队列（简单 append + headPointer）</li>
         *   <li>bad 文件（.bad）：记录不可读的 offset 以便排查</li>
         * </ul>
         */
    ...
    static class DbMasterIndex extends DsObject {
        // 每个 Bucket 存储: Key(8B) + Value(8B) = 16B
        ...
        /**
         * 创建一个总控索引文件（MD5 前缀 -> indexId）。
         *
         * <p>该索引用 64bit key（取 md5 的低 8 字节）做线性探测哈希表。</p>
         */
        ...
        public void put(byte[] md5, long indexId) throws IOException {
            long key = DsDataUtil.loadLong(md5, 0);
            long hash = Math.abs(key) % capacity;
            ...
            while (true) {
                ...
                long storedKey = buffer.getLong(bufferOffset);
                long storedVal = buffer.getLong(bufferOffset + 8);
                
                if (storedKey == 0 && storedVal == 0) {
                    // Empty slot, insert here
                    buffer.putLong(bufferOffset, key);
                    buffer.putLong(bufferOffset + 8, indexId);
                    dirty(bufferIndex);
                    return;
                }
