...
//        long bufferIndex = levelOffset / BLOCK_SIZE;
        ...
        try {
            if (levelUsed < levelCapacity) {//当前层有空位
                storeLong(levelOffset + dataLayerDescSize + (long) levelUsed * dataUnitSize, value);
                levelUsed++;
                storeInt(levelOffset + usedIndex, levelUsed);
                return true;
            } else {
                // 当前层已满，需要扩容
                level++;
                if (level >= dataLayers.size()) {
                    ...
                    storeInt(levelOffset + rightOffsetIndex, (int) (basePtrNext >>> 3));
            ...
            if (rest >= count) {
                ...
                while (allRest > 0) {
                    ...
                    if (level >= dataLayers.size()) {
                        ...
                        layer = layerNew;
                        storeInt(levelOffset, (int) (layerNew.levelOffset >> 3));//左指针偏移 8字节对齐
                        storeInt(levelOffset + usedIndex, layerNew.used);
                        storeInt(levelOffset + capacityIndex, layerNew.capacity);
                        //其他zero
                        storeInt(levelOffset + rightOffsetIndex, (int) layerNew.rightOffset>>3);
                    } else {
                        layer = dataLayers.get(level);
                    }
    ...
    private boolean storeValueWithNextBlock(long basePtrNext, long value) throws IOException {
        ...
        layer.rightOffset = 0;
        dataLayers.add(layer);
        storeInt(levelOffset, (int) (layer.levelOffset >> 3));//左指针偏移 8字节对齐
        storeInt(levelOffset + usedIndex, layer.used);
        storeInt(levelOffset + capacityIndex, layer.capacity);
        //其他zero
        storeInt(levelOffset + rightOffsetIndex, (int) layer.rightOffset>>3);

        storeLong(levelOffset + dataLayerDescSize, value);
        return true;
    ...
    private DataLayer newDataBlock(long basePtrNext, int levelCapacity) throws IOException {
        ...
        storeInt(basePtrNext, (int) (layer.levelOffset >> 3));//左指针偏移 8字节对齐
        storeInt(basePtrNext + usedIndex, layer.used);
        storeInt(basePtrNext + capacityIndex, layer.capacity);
        //其他zero
        storeInt(basePtrNext + rightOffsetIndex, 0);
        return layer;
                ...
                if (currentLayer.used > 0) {
                    ...
                    for (int i = 0; i < currentLayer.used; i++) {
                        ...
                        if (v == value) {
                            currentLayer.used--;
                            minusTotal(1);
                            storeInt(currentLayer.levelOffset + usedIndex, currentLayer.used);
                    ...
                    if (index <= (currentIndex + layer.used - 1)) {
                        ...
                        if (rest >= count) {
                            ...
                            while (allRest > 0) {
                                ...
                                if (allRest > 0) {
                                    layerX.used = 0;
                                    storeInt(layerX.levelOffset + usedIndex, layerX.used);
                ...
                if (layer.used > 0 || index == 0) {
                    if (index <= (currentIndex + layer.used - 1) || index == 0) {
                        ...
                        if (layer.used < layer.capacity) {
                            ...
                            storeInt(layerNew.levelOffset + rightOffsetIndex, (int) (layerNew.rightOffset >> 3));
                            if (layerIndex > 0) {//如果不是队列头部,更新levelOffset
                                ...
                                storeInt(layerPrev.levelOffset + rightOffsetIndex, (int) (layerPrev.rightOffset >> 3));
                            }
                            ...
                            storeInt(layer.levelOffset + rightOffsetIndex, (int) (layer.rightOffset >> 3));
                            dataLayers.add(layerIndex, layerNew);
                            return;
                        }
            ...
            for (DataLayer layer : dataLayers) {
                if (layer.used > 0) {
                    if (index <= (currentIndex + layer.used - 1)) {
                        ...
                        try {
                            int i = index - currentIndex;
                            storeLong(layer.levelOffset + dataLayerDescSize + (long) i * dataUnitSize, value);
                            //storeInt(layer.levelOffset + usedIndex, layer.used);
                            return;
                        } finally {
                            layer.lock.writeLock().unlock();
