 ...
 *   <li>路径统一 Linux 风格：绝对路径以 '/' 开头；系统空间以 '$' 为第 1 字符（例如 "$Database"）。</li>
 ...
 *   <li>group：工作组（按业务规则映射到此空间）。</li>
 *   <li>private：私域（按业务规则映射到此空间）。</li>
 ...
 *   <li>dirStore：{@link Ds256DirectoryStore}，dirId -> entryId 列表（目录成员表）。</li>
 *   <li>tagIndex：{@link DsTagIndex}，tag -> fileId 列表。</li>
 * </ul>
 */
public class Ds256FileSystem {

    private static final String NS_GLOBAL = "global";//全局文件空间。
    private static final String NS_GROUP = "group";//主共享文件空间。
    private static final String NS_PRIVATE = "private";//私有域文件空间。
     ...
     * @param rootDir 存储根目录
     */
    public Ds256FileSystem(String rootDir) throws IOException {
        this.globals = new Stores(rootDir, NS_GLOBAL);
        this.groups = new Stores(rootDir, NS_GROUP);
        this.privates = new Stores(rootDir, NS_PRIVATE);
        this.systems = new Stores(rootDir, NS_SYSTEM);
    }

    private final Stores globals;
    private final Stores groups;
    private final Stores privates;
    private final Stores systems;
    ...
    public List<FileMetadata.BasicMeta> getFilesByTag(String tagKey, String tagValue)
        ...
        return getFilesByTagInStores(globals, tagKey, tagValue);
    }
    ...
    public FileMetadata getMetadata(String globalFileId) throws IOException, InterruptedException, ClassNotFoundException {
        long fileId = getFileIdByGlobalFileId(globals, globalFileId);
        if (fileId == 0) {
            return null;
        }
        return loadMetadataByFileId(globals, fileId, globalFileId);
    }
    ...
    public FileMetadata.BasicMeta getBasicMetadata(String globalFileId)
        ...
        return loadBasicByFileId(globals, fileId);
    }
    ...
    public byte[] getFileContent(String contentHashStr) throws IOException, InterruptedException {
        return getFileContent(globals, contentHashStr);
    }
    ...
    private long getOrCreateFileIdByGlobalFileId(Stores s, String globalFileId) throws IOException {
        ...
        long fileId = Ds256DirectoryStore.bytesToLong(v);
        if (fileId != 0) {
            return fileId;
        }
    ...
    private long getFileIdByGlobalFileId(Stores s, String globalFileId) throws IOException {
        ...
        return Ds256DirectoryStore.bytesToLong(v);
    }
    ...
    private long getOrCreateFileIdByPath(Stores s, String path) throws IOException {
        ...
        fileId = s.dirStore.allocateFileId();
        s.pathToFileId.update(k, Ds256DirectoryStore.longToBytes(fileId));
        return fileId;
    }
    ...
    private long ensureDirPath(Stores s, String dirPath) throws IOException {
        ...
        if (p.equals("/")) {
            ...
            long rootId = Ds256DirectoryStore.bytesToLong(v);
            if (rootId != 0) {
                return rootId;
            }
            rootId = s.dirStore.allocateDirId();
            s.pathToDirId.update("/".getBytes(StandardCharsets.UTF_8), Ds256DirectoryStore.longToBytes(rootId));
            return rootId;
        }
        ...
        for (String seg : segs) {
            ...
            if (dirId == 0) {
                ...
                s.pathToDirId.update(current.getBytes(StandardCharsets.UTF_8), Ds256DirectoryStore.longToBytes(dirId));
    ...
    private Stores stores(String ns) {
        if (NS_SYSTEM.equals(ns)) {
            return systems;
        }else if (NS_GROUP.equals(ns)) {
            return groups;
        }else if (NS_PRIVATE.equals(ns)) {
            return privates;
        }
        return globals;
    }
    ...
    private static ParsedPath parsePath(ParsedPath parent, String path) {
        ...
        if (path.startsWith("$")) {
            if (path.equals("$")) {
                return new ParsedPath(NS_SYSTEM, "/");
            }
            String rest = path.substring(1);
            if (rest.startsWith("/")) {
                rest = rest.substring(1);
            }
            String normalized = DsPathUtil.normalizeLinuxPath(ensureLeadingSlash(rest), true);
            return new ParsedPath(NS_SYSTEM, normalized);
        }
        
        if (path.startsWith("group://")) {
            String normalized = DsPathUtil.normalizeLinuxPath(ensureLeadingSlash(path.substring("group://".length())), true);
            return new ParsedPath(NS_GROUP, normalized);
            
        }
        if (path.startsWith("private://")) {
            String normalized = DsPathUtil.normalizeLinuxPath(ensureLeadingSlash(path.substring("private://".length())), true);
            return new ParsedPath(NS_PRIVATE, normalized);
        }
        if (parent == null) {
