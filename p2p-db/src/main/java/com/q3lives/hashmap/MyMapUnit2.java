...
public class MyMapUnit2<K, V> implements Serializable {
    ...
    V put(K key, V value) {
        ...
        if (table[hash] == null) {
            ...
            if (table[hash] instanceof KeyValue) {
                ...
                MyMapUnit2<K, V> mmu = new MyMapUnit2<K, V>();
            ...
            MyMapUnit2<K, V> mmu = (MyMapUnit2<K, V>) table[hash];
    ...
    V put(KeyValue<K, V> pair, int code, int layer) {
        ...
        if (table[hash] == null) {
            ...
            return ((MyMapUnit2<K, V>) table[hash]).put(pair, code, layer + 1);
    ...
    V get(Object key) {
        ...
        if (table[hash] != null) {
            if (table[hash] instanceof KeyValue) {
                ...
                return ((MyMapUnit2<K, V>) table[hash]).get(key, code, layer + 1);
//				MyMapUnit<K, V> mmu = (MyMapUnit<K, V>)table[hash];
//				
        ...
        for (int i = 8; i < 16; i++) {//首先处理负整数集
            if (table[i] != null) {
                if (table[i] instanceof KeyValue) {
                    ...
                    MyMapUnit2<K, V> mmu = (MyMapUnit2<K, V>) table[i];
    ...
    V remove(K key) {
        ...
        if (table[hash] != null) {
            if (table[hash] instanceof KeyValue) {
                ...
                return ((MyMapUnit2<K, V>) table[hash]).remove(key, code, layer + 1);
//				MyMapUnit<K, V> mmu = (MyMapUnit<K, V>)table[hash];
//				
//				return mmu.get(key,bytes,layer+1);
            }
