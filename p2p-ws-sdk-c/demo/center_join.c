...
int main(int argc, char** argv) {
  ...
  if (p2pws_cipher_prepare_hand(&cfg, &rsa, need_keyfile ? key_id32 : NULL, &ctx, &msg) != 0) {
  ...
  if (p2pws_cipher_apply_hand_ack(&hak, &ctx) != 0) {
    printf("hand_ack_apply_failed\n");
    return 17;
  }
  ...
  if (p2pws_cipher_apply(&ctx, need_keyfile ? &kf : NULL, wrap.data, wrap.len, cipher.data) != 0) {
    printf("xor_failed\n");
    return 22;
  }
  cipher.len = wrap.len;
  if (send_wire_frame(&ws, cfg.magic, (uint8_t)cfg.version, p2pws_cipher_wire_flags(&cfg, &ctx), cipher.data, cipher.len) != 0) {
  ...
  if (p2pws_cipher_apply(&ctx, need_keyfile ? &kf : NULL, cipher.data, cipher.len, plain.data) != 0) {
