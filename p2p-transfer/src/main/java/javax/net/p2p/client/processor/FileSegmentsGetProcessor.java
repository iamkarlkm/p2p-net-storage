...
public class FileSegmentsGetProcessor implements Runnable {
	...
	public FileSegmentsGetProcessor(P2PFileService node, FileSegmentsDataModel model, File data,RandomAccessFile infoFile, CountDownLatch countDownLatch, CountDownLatch errorCountDown, AtomicBoolean errorTag) {
		this.node = node;
		this.model = model;
	...
	public void run() {
		try {
			...
			Logger.getLogger(FileSegmentsGetProcessor.class.getName()).log(Level.SEVERE, null, ex);
			errorTag.set(true);
		} finally {
