...
| 9 | **Header Tiered WAL / OceanBase 风格日切合并已交付（2026-08-12，三模式 3×142 全回归 PASS）**：只对 64KB 文件头（superblock/metadata only）做日切 delta 分层，数据本体（ID>0）不做 WAL，规避「一份数据两次写盘」的写放大。三模式开关 `HeaderTierMode.DIRECT|MEM_DELTA|FILE_DELTA`；当日 delta 独立存储（MEM=BitSet 级 byte COW 独立副本；FILE=独立 `<name>.hdr_tier` 文件含 MAGIC+BitSet+64KB block，支持 crash 后自动恢复重建 base MMAP 最新值）；三查/双查读取策略（new → 合并中 → base，三选最新 byte）；`DailyMergeService` 单例每 3 AM 默认 rollover，状态机 IDLE→MERGING→SWAP_PENDING→IDLE，注册/关闭零侵入 | **已达生产级最小可用**：配置开关 `DSDB_HEADER_TIER_MODE` 环境变量/Sys prop 三选一切换，对业务代码零改动，DsHashMap/DsHashSet/DsList/DsBlockManager/DsFixedBucketStore/DsDataStore/Ds*MasterIndex/DsDataIndex 13 类全部继承生效（DIR/MEM/FILE 三模式各自 142/142 测试全绿 + Range 9/9 全绿）| **新文件**：`com/q3lives/ds/database/config/DsDbConfig.java` + `com/q3lives/ds/header/{HeaderTieredStore,DirectHeaderTierStore,MemDeltaHeaderTierStore,FileDeltaHeaderTierStore,DailyMergeService,HeaderTieredStoreFactory}.java`；**注入点**：`DsObject.java`/`DsMemory.java` 构造函数 `headerTier = Factory.create(dataFile,name)` + `ensureHeaderTierAttached/markHeaderFieldDirty/markHeaderFullDirty`；**验证**：DIRECT(142) + MEM_DELTA(142) + FILE_DELTA(142) = 426 tests 全 PASS |
