...
| 8 | **DsHashMap 范围查询 public API 已交付（2026-08-10, 5/5 Range PASS + 139/139 全回归）**：已补齐 NavigableMap 风格 API — `range(start,count)`（按全局有序 index 分页）、`forEachRange/forEachKeyRange`（带 inclusive 开关 + limit 的 stream 原语）、`subMap/tailMap/headMap(...,limit)`、`gt/gte/lt/lte(key,limit)`、`ceilingKey/higherKey/floorKey/lowerKey`、`firstKey/lastKey`；全部 100% 与 `TreeMap<Long,Long>` 金参对齐（覆盖 16/32/64 级跨段边界 ±1、Long.MIN/MAX、空图/单例、seqno 类时间范围等 5 测试 21 assertions）| **正确性层无阻塞**：p2p-sync 时间范围回放可直接 `gte(seqnoFloor, limit)` 或 `tailMap(seqnoFloor, true, limit)` 拿到升序最新段；seqno 单调场景需优化尾查询性能时，后续可再把现有 `forEachKeyRange` 从「iterator 扫 → key 过滤」升级到内部 O(log N + count) 级别 seek（需先对齐 get/put 的 key 分布不对称：get 按 key 范围硬跳级，put 依赖 STATE_NEXT_LEVEL 冲突升级，二者分配不一致时 iterator 扫反而更稳）| 新增 `DsHashMap.java: forEachKeyRange()/forEachRange()` + 配套 API；测试见 `DsHashMapRangeTest.java`（5 金参对齐测试 + DsHashMapTest/DsHashSetTest/DsMemoryBulkArrayTest 全回归 PASS） |
