...
| 9 | **Header Tiered WAL / OceanBase 风格日切合并已交付（2026-08-13，四模式 4×142=568 全回归 PASS + 三模式 Range 27/27 全绿）**：只对 superblock/metadata 文件头做日切 delta 分层，数据本体（ID>0）0 次额外写盘，规避「一份数据两次写盘」写放大。**三档开关** `HeaderTierMode.DIRECT|MEM_DELTA|FILE_DELTA`；**FILE_DELTA 两子模式**：① legacy 独立小文件（每 store 单独 `<name>.hdr_tier` 64KB 同构块，含 MAGIC+BitSet，crash 自动重放）② **shared 合并 4KB 页日志（方案 B 已交付）**：相对路径 `intern` 到 `FileStoreIdRegistry` 得到递增紧凑 long storeId → 同一 `delta_headers_<dayKey>.log` 内多 store 打包，**4KB 页 = 32B 页头 + 7×(24B slot 头 + 512B payload)=7 个 store 头**，SSD 页对齐；slot 内置 CRC16 / 页 CRC32 防坏数据，crash 全量页重放重建 base。`DSDB_HEADER_SHARED_LOG=1/true` 切 shared；系统 store（ID 注册表自身）fallback 独立 .hdr_tier 破 chicken-egg。双查（IDLE：new_today + base）/ 三查（MERGING：new_today + Δ_merging + base）模型齐全。`DailyMergeService` 单例每 3 AM 后台 rollover 合并线程，状态机 IDLE→MERGING→SWAP_PENDING→IDLE，注册/关闭零侵入 | **已达生产级**：配置开关（环境变量/Sys prop 二选一，子模式子开关正交）对业务代码零改动，DsHashMap/DsHashSet/DsList/DsBlockManager/DsFixedBucketStore/DsDataStore/Ds*MasterIndex/DsDataIndex 13 类全部继承生效；4KB 页 slot 默认 512B 完全覆盖所有集合真实头字段（DsFreeRing=20B、DsList<=96B、DsHashMap/DsFixedBucketStore<=~256B，均 << 512B）| **新文件**：`com/q3lives/ds/database/config/DsDbConfig.java`（新增 ENV_HEADER_SHARED_LOG、REGISTRY_SUB_DIR、getOrCreateStoreIdRegistry）+ `com/q3lives/ds/header/{HeaderTieredStore,DirectHeaderTierStore,MemDeltaHeaderTierStore,FileDeltaHeaderTierStore,SharedHeaderLogLayout,StoreIdRegistry,FileStoreIdRegistry,SharedHeaderDeltaLog,SharedLogFileDeltaHeaderTierStore,DailyMergeService,HeaderTieredStoreFactory}.java`；**注入点**：`DsObject.java`/`DsMemory.java` 构造函数 `headerTier = Factory.create(dataFile,name)` + `ensureHeaderTierAttached/markHeaderFieldDirty/markHeaderFullDirty`；**验证**：DIRECT(142) + MEM_DELTA(142) + FILE_DELTA legacy(142) + FILE_DELTA shared(142) = 568 tests 全 PASS，三模式各 9/9 Range 全绿 TailSeek ratio=1 |
