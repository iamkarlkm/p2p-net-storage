...
public class SimpleChannelPool implements ChannelPool {
    ...
    private Future<Channel> acquireHealthyFromPoolOrNew(final Promise<Channel> promise) {
        try {
            ...
            System.out.println("acquireHealthyFromPoolOrNew "+ch);
            if (ch == null) {
    ...
    private void doHealthCheck(final Channel channel, final Promise<Channel> promise) {
        try {
            ...
            System.out.println("doHealthCheck cause "+cause);
            closeAndFail(channel, cause, promise);
        }
    }
        ...
        try {
            ...
            if (future.isSuccess() && future.getNow()) {
                ...
                promise.setSuccess(channel);
                 ...
                 System.out.println("notifyHealthCheck closeChannel(channel) "+future.getNow());
    ...
    private void doReleaseChannel(Channel channel, Promise<Void> promise) {
        try {
            ...
            System.out.println(channel.attr(POOL_KEY));
            if (channel.attr(POOL_KEY).getAndSet(null) != this) {
                System.out.println("closeAndFail");
                ...
                if (releaseHealthCheck) {
                    System.out.println("doHealthCheckOnRelease");
                    doHealthCheckOnRelease(channel, promise);
                } else {
                    System.out.println("releaseAndOffer");
    ...
    private void doHealthCheckOnRelease(final Channel channel, final Promise<Void> promise) throws Exception {
        ...
        if (f.isDone()) {
            System.out.println("releaseAndOfferIfHealthy");
    ...
    private void releaseAndOfferIfHealthy(Channel channel, Promise<Void> promise, Future<Boolean> future) {
        try {
            ...
            System.out.println("closeAndFail -> "+cause.getMessage());
            closeAndFail(channel, cause, promise);
        }
    }
    ...
    private void releaseAndOffer(Channel channel, Promise<Void> promise) throws Exception {
        if (offerChannel(channel)) {
            System.out.println("offerChannel(channel) -> "+deque.size());
            handler.channelReleased(channel);
            promise.setSuccess(null);
        } else {
            System.out.println("releaseAndOffer closeAndFail -> "+deque.size());
    ...
    protected Channel pollChannel() {
        ...
        System.out.println("lastRecentUsed" +lastRecentUsed);
        Channel c = lastRecentUsed ? deque.pollLast() : deque.pollFirst();
