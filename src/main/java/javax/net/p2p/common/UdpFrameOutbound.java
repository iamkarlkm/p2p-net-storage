 ...
 */
@Slf4j
public class UdpFrameOutbound extends PooledableAdapter implements Closeable,Runnable{
...
//    }
    ...
    public final static <T> UdpFrameOutbound build(AbstractUdpMessageProcessor messageProcessor,Channel channel,InetSocketAddress remoteAddess,int magic,Integer queueSize) {
        UdpFrameOutbound t = ConcurrentObjectPool.get().poll();
    ...
    public final static <T> UdpFrameOutbound build(AbstractUdpMessageProcessor messageProcessor,Channel channel,InetSocketAddress remoteAddess,int headerSize,int magic,Integer queueSize) {
        UdpFrameOutbound t = ConcurrentObjectPool.get().poll();
        t.messageProcessor = messageProcessor;
        t.channel = channel;
        t.remoteAddess = remoteAddess;
...
//                System.out.println("Server echo -> test udp-0".length());
    ...
    public void start()  {
       this.future =  ExecutorServicePool.SERVER_ASYNC_POOLS.submit(this);
    }
    ...
    static class ConcurrentObjectPool {
        ...
        private static final ThreadLocal<PooledObjects<UdpFrameOutbound>> LOCAL_POOL = new ThreadLocal<>();

        private static PooledObjects<UdpFrameOutbound> get() {
            PooledObjects pool = LOCAL_POOL.get();
            if (pool == null) {
                pool = new PooledObjects(4096, new PooledObjectFactory<UdpFrameOutbound>() {
                    @Override
                    public UdpFrameOutbound newInstance() {
                        return new UdpFrameOutbound();
                    }
                });
                LOCAL_POOL.set(pool);
            }
