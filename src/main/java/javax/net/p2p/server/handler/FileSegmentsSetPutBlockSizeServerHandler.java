 ...
 */
public class FileSegmentsSetPutBlockSizeServerHandler implements P2PCommandHandler {

	@Override
	public P2PCommand getCommand() {
		return P2PCommand.SET_FILE_SEGMENTS_PUT_BLOCK_SIZE;
	}
    
    @Override
    public P2PWrapper process(P2PWrapper request) {
        try {
            if (request.getCommand().getValue() == P2PCommand.SET_FILE_SEGMENTS_PUT_BLOCK_SIZE.getValue()) {
							FileSegmentsDataModel payload = (FileSegmentsDataModel) request.getData();
               P2PConfig.DATA_PUT_BLOCK_SIZE = payload.blockSize;
